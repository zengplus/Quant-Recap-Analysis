""" 
 File: engine.py
 Date: 2026-01-17
 Description: 通用 Alpha 挖掘引擎。定义了 AlphaEngine 类，包含模型初始化、优化器配置（支持 LoRD 正则化）和基础训练逻辑，主要用于挖掘 Alpha 因子。
 """ 
import torch
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            for _ in range(ModelConfig.MAX_FORMULA_LEN):
                ...
                # 检查 logits 是否含有 NaN，如果有则替换为 0 或极小值
                if torch.isnan(logits).any():
                    # print("Warning: NaN in logits, replacing with 0.")
        ...
        with open("outputs/best_meme_strategy.json", "w") as f:
            json.dump(self.best_formula, f)

        # Save model weights
        torch.save(self.model.state_dict(), "outputs/alphagpt_model.pth")
